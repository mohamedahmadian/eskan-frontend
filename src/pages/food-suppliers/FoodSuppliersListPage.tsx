...
export function FoodSuppliersListPage() {
  ...
  const query = useQuery({
    queryKey: ['food-suppliers', 'list', q, provinceId, cityId, page],
    queryFn: async () => {
  ...
  return (
    <div className={listShellClassName}>
      ...
      <AppForm onSubmit={onSearch} className={`mb-4 grid gap-4 p-4 sm:grid-cols-3 ${cardClassName}`}>
        ...
        <FormField icon={Search} label={t('foodSuppliers.search')} htmlFor="supplier-search">
      ...
      <TableCard loading={query.isLoading} empty={emptyMessage} hasRows={rows.length > 0}>
        <table className="w-full text-sm">
          <thead className="bg-cream-50 text-ink-700">
            <tr>
              <th className="px-4 py-3 text-start font-medium">{t('foodSuppliers.name')}</th>
              ...
              <th className="px-4 py-3 text-start font-medium">{t('foodSuppliers.phone')}</th>
