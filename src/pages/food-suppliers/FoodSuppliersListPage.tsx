...
import type { City, FoodSupplier, Paginated, Province } from '../../types/app'

export function FoodSuppliersListPage() {
  ...
  const query = useQuery({
    queryKey: ['food-suppliers', 'list', q, provinceId, cityId, page, sortBy, sortDir],
    queryFn: async () => {
      const { data } = await api.get<Paginated<FoodSupplier>>('/food-suppliers', {
        params: {
          page,
          ...(q ? { q } : {}),
  ...
  const createTo = filterQuery
    ? `/base-info/food-suppliers/new?${filterQuery}`
    : '/base-info/food-suppliers/new'
  const rows = query.data?.items ?? []
  const emptyMessage = q || provinceId || cityId ? t('foodSuppliers.noResults') : t('foodSuppliers.empty')
  ...
  return (
    <div className={listShellClassName}>
      ...
      <SearchBar
        ...
        onSubmit={onSearch}
        label={t('foodSuppliers.search')}
        placeholder={t('foodSuppliers.searchPlaceholder')}
        filtersActive={Boolean(provinceId || cityId)}
        extra={
          <FilterPair>
            <FormField icon={Filter} label={t('geo.province')} htmlFor="supplier-province">
              <SearchSelect
            ...
            <FormField icon={Filter} label={t('geo.city')} htmlFor="supplier-city">
              <SearchSelect
                id="supplier-city"
                value={cityId}
                disabled={!provinceId}
      ...
      <TableCard loading={query.isLoading} empty={emptyMessage} hasRows={rows.length > 0}>
        <table className="w-full text-sm">
          <thead className="bg-cream-50 text-ink-700">
            <tr>
              <SortableTh column="name" label={t('foodSuppliers.name')} sortBy={sortBy} sortDir={sortDir} onSort={onSort} />
              ...
              <SortableTh column="phone" label={t('foodSuppliers.phone')} sortBy={sortBy} sortDir={sortDir} onSort={onSort} />
            ...
            {rows.map((item) => (
              <tr key={item.id} className="border-t border-line">
                ...
                <td className="px-4 py-3">
                  <EntityRowActions
                    viewTo={`/base-info/food-suppliers/${item.id}`}
