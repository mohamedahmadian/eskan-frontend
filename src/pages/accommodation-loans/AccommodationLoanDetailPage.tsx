...
import { formatItemUnit, type AccommodationLoan } from '../../types/app'
import { DetailRow } from '../geo/GeoShared'

export function AccommodationLoanDetailPage() {
  ...
  return (
    <div className={formShellClassName}>
      <PageHeader
        ...
        subtitle={t('accommodationLoans.detailsSubtitle')}
        action={
          <Link to="/logistics/loans" className="text-sm text-teal-700 hover:underline">
            {t('accommodationLoans.backToList')}
          </Link>
        }
      />
      <article className="rounded-2xl border border-line bg-white p-6 shadow-sm">
        <dl className="grid gap-3 text-sm">
          ...
          <DetailRow
            ...
            value={`${item.accommodationManager.fullName} — ${item.accommodationManager.username}`}
          />
          <DetailRow
            label={t('accommodationLoans.quantity')}
            value={`${formatNumber(item.quantity, locale)} ${formatItemUnit(item.supplierItem.unit, t)}`}
          />
          <DetailRow
        ...
        <DetailActions
          ...
          deleteLabel={t('accommodationLoans.delete')}
          onDelete={() =>
            confirmDelete({
              ...
              onDeleted: () => navigate('/logistics/loans'),
            })
          }
        />
      </article>
