...
import type { ManagedUser, OrgUnitManager, Paginated } from '../../types/app'

export type UnitManagerChoice = OrgUnitManager

function toChoice(user: ManagedUser): UnitManagerChoice {
  return {
    id: user.id,
...
export function UnitManagerPicker({
  value,
  onChange,
}: {
  value: UnitManagerChoice | null
  onChange: (next: UnitManagerChoice | null) => void
}) {
  ...
  const query = useQuery({
    queryKey: ['users', 'unit-manager-picker', q, page],
    enabled: Boolean(q),
  ...
  function runSearch() {
    ...
    if (!next) {
      toast.error(t('orgUnits.managerSearchRequired'))
      return
    }
    setQ(next)
    setPage(1)
  ...
  return (
    <div className="relative space-y-3" data-enter-ignore>
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="flex items-center gap-2 font-medium">
                ...
                {t('orgUnits.managerSelected', { name: value.fullName })}
              </p>
            ...
            <Button
              ...
              aria-label={t('orgUnits.clearManager')}
              title={t('orgUnits.clearManager')}
      ...
      <FormField icon={UserRound} label={t('orgUnits.manager')} htmlFor="unit-manager-search">
        <div className="flex w-full flex-col gap-3 sm:flex-row">
          <input
            id="unit-manager-search"
            className={`${fieldClassName} min-w-0 flex-1`}
            value={term}
            onChange={(event) => setTerm(event.target.value)}
            onKeyDown={onSearchKeyDown}
            placeholder={t('orgUnits.managerSearchPlaceholder')}
          />
      ...
      {q ? (
        <>
          {query.isError ? (
            ...
            <TableCard
              ...
              empty={t('orgUnits.managerSearchNoResults')}
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            ...
                            >
                              ...
                              {selected
                                ? t('orgUnits.managerChosen')
                                : t('orgUnits.selectAsManager')}
                            </Button>
                          </td>
                        </tr>
                      )
        ...
        <p className="text-sm text-ink-500">{t('orgUnits.managerSearchHint')}</p>
      )}
    </div>
  )
}
