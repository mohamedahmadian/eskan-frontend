...
export function MedicalCentersListPage() {
  ...
  const query = useQuery({
    queryKey: ['medical-centers', 'list', q, provinceId, cityId, page, sortBy, sortDir],
    queryFn: async () => {
      ...
      <TableCard loading={query.isLoading} empty={emptyMessage} hasRows={rows.length > 0}>
        <table className="w-full text-sm">
          <thead className="bg-cream-50 text-ink-700">
            <tr>
              <SortableTh column="name" label={t('medicalCenters.name')} sortBy={sortBy} sortDir={sortDir} onSort={onSort} />
              ...
              <SortableTh column="phone" label={t('medicalCenters.phone')} sortBy={sortBy} sortDir={sortDir} onSort={onSort} />
