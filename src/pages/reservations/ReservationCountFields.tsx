import { Check, Mars, Minus, Plus, UserRound, Users, Venus } from 'lucide-react'
...
function CountMetricTile({
  ...
  return (
    <article className={`flex h-full flex-col items-center gap-1.5 rounded-2xl border px-2 py-3 text-center ${colors.wrap}`}>
      ...
      {editable ? (
        <input
          ...
          max={max}
          required
          className="w-full border-0 bg-transparent text-center text-lg font-bold leading-none text-ink-900 outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
          value={value}
          onChange={(event) => {
        ...
        <p className="text-lg font-bold leading-none text-ink-900">{displayValue}</p>
      )}
      {editable ? (
        <div dir="ltr" className="flex items-center gap-1.5">
          <button
            ...
            disabled={atMin}
            onClick={() => step(-1)}
            className={`flex size-7 items-center justify-center rounded-lg transition ${colors.btn} disabled:cursor-not-allowed disabled:opacity-60`}
          >
