import { Building, ChartColumn, HandHeart, Phone, Plus, type LucideIcon } from 'lucide-react'
...
export function GovernmentOrgOfficerDashboard() {
    ...
    <div className={`${listShellClassName} space-y-8`}>
      <section className={`${cardClassName} overflow-hidden`}>
        ...
        <div className="flex items-start gap-3 px-5 py-5">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-ink-500">{t('dashboard.supportOfficerSubtitle')}</p>
          </div>
        </div>
      </section>

      <section className="space-y-4">
        <h2 className="text-sm font-medium text-ink-500">{t('dashboard.supportOfficerOrganization')}</h2>
      ...
      <section>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <ActionCard
            to="/support-requests/new"
            icon={Plus}
            label={t('supportRequests.create')}
            tone="teal"
          />
          <ActionCard
            to="/support-requests"
            icon={HandHeart}
            label={t('menus.supportRequests')}
            tone="mint"
          />
          <ActionCard
            to="/support-request-report"
            icon={ChartColumn}
