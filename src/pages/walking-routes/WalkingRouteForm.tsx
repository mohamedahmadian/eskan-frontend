...
export function WalkingRouteForm({
  ...
  const [entryBorderCityId, setEntryBorderCityId] = useState(initial?.entryBorderCityId ?? '')
  ...
  return (
    <AppForm onSubmit={submit} className="space-y-4">
      <FormSection title={t('walkingRoutes.sectionGeneral')}>
        ...
        <FormField icon={Flag} label={t('walkingRoutes.entryBorder')} htmlFor="entry-border">
          <SearchSelect
            id="entry-border"
            value={entryBorderCityId}
            required
            placeholder={t('walkingRoutes.selectEntryBorder')}
            onChange={setEntryBorderCityId}
            options={citySelectOptions(
              iranCities,
              geoName,
              t('walkingRoutes.selectEntryBorder'),
            )}
          />
        </FormField>
      </FormSection>

      <FormSection title={t('walkingRoutes.sectionOriginCountries')}>
        <FormField icon={Flag} label={t('walkingRoutes.originCountries')}>
      ...
      <FormSection title={t('walkingRoutes.sectionStages')}>
        <div className="space-y-4">
          {stages.map((stage, index) => (
            <article
              key={stage.key}
              className="space-y-4 rounded-2xl border border-line bg-cream-50 p-4"
            >
          ...
          <Button type="button" variant="ghost" onClick={() => setStages([...stages, emptyStage()])}>
